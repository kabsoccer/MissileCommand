...
var defaultEye = vec3.fromValues(1,0.0,-1.5); // default eye position in world space
var defaultCenter = vec3.fromValues(1,0.0,0.5); // default view direction in world space
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
       // Keyboard input not supported for this assignment
    } // end switch
} // end handleKeyDown
...
function renderModels() {
	...
	mat4.ortho(pMatrix, 0.0, 1.0, 0.0, 1.0, 0.1, 10);
    ...
    for (var i = 0; i < enemyMissiles.length; i++) {
		...
		for (var j = 0; j < buildings.length; j++) {
			if ((enemyMissiles[i][0] <= buildings[j][0] + 0.05 && enemyMissiles[i][0] >= buildings[j][0] - 0.05) &&
				(enemyMissiles[i][1] <= buildings[j][1] + 0.1 && enemyMissiles[i][1] >= buildings[j][1])) {
		...
		for (var j = 0; j < silos.length; j++) {
			if ((enemyMissiles[i][0] <= silos[j][0] + 0.05 && enemyMissiles[i][0] >= silos[j][0] - 0.05) &&
				(enemyMissiles[i][1] <= silos[j][1] + 0.1 && enemyMissiles[i][1] >= silos[j][1])) {
				silos[j] = -1;
				enemyMissiles[i][0] = Math.random();
				enemyMissiles[i][1] = Math.random() * 2 + 1;
				enemyMissiles[i][2] = 0;
    ...
    } // end for each enemy missile
	
	// render each friendly missile
	for (var i = 0; i < friendlyMissiles.length; i++) {
        ellipsoid = inputEllipsoids[1];
		
		//inputEllipsoids[1].center = friendlyMissiles[i];
		inputEllipsoids[1].translation = vec3.fromValues(friendlyMissiles[i][0], friendlyMissiles[i][1], friendlyMissiles[i][2]);
		...
		if (ellTextures[1] == -1) {
			...
			gl.uniform1i(texModeULoc, textureMode);
			gl.activeTexture(gl.TEXTURE0);
			gl.bindTexture(gl.TEXTURE_2D, ellTextures[1].texture);
			gl.uniform1i(textureULoc, 0);
		}
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffers[numTriangleSets+1]); // activate vertex buffer
        gl.vertexAttribPointer(vPosAttribLoc,3,gl.FLOAT,false,0,0); // feed vertex buffer to shader
        gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[numTriangleSets+1]); // activate normal buffer
        gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed normal buffer to shader
        gl.bindBuffer(gl.ARRAY_BUFFER,UVBuffers[numTriangleSets+1]); // activate
        gl.vertexAttribPointer(vTexAttribLoc,2,gl.FLOAT,false,0,0); // feed
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,triangleBuffers[numTriangleSets+1]); // activate tri buffer
	...
	for (var i = 0; i < explosions.length; i++) {
        ellipsoid = inputEllipsoids[2];
		
		//inputEllipsoids[1].center = friendlyMissiles[i];
		inputEllipsoids[2].translation = vec3.fromValues(explosions[i][0], explosions[i][1], 0);
		...
		if (ellTextures[2] == -1) {
			...
			gl.uniform1i(texModeULoc, textureMode);
			gl.activeTexture(gl.TEXTURE0);
			gl.bindTexture(gl.TEXTURE_2D, ellTextures[2].texture);
			gl.uniform1i(textureULoc, 0);
		}
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffers[numTriangleSets+2]); // activate vertex buffer
        gl.vertexAttribPointer(vPosAttribLoc,3,gl.FLOAT,false,0,0); // feed vertex buffer to shader
        gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[numTriangleSets+2]); // activate normal buffer
        gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed normal buffer to shader
        gl.bindBuffer(gl.ARRAY_BUFFER,UVBuffers[numTriangleSets+2]); // activate
        gl.vertexAttribPointer(vTexAttribLoc,2,gl.FLOAT,false,0,0); // feed
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,triangleBuffers[numTriangleSets+2]); // activate tri buffer
